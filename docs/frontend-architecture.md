...
- כל קובץ runtime יישומי תחת `src/js` ו־`src/runtime` הוא ES Module אמיתי. כל Route מתחיל ב־entrypoint קטן תחת `src/entries`; esbuild אורז את גרף ה־Feature למסלול ומשאיר את שירותי `src/runtime` כ־external ESM assets משותפים ונפרדים. התוצרים נטענים דרך import graph של `type="module"`, ללא globals עסקיים, מעטפת IIFE ברמת ה־bundle או loader תאימות. IIFE מקומי המשמש לאתחול ערך פנימי אינו שכבת תאימות ואינו משנה את גבול המודול. נתוני generated ו־`catalog-assets.config.js` נמצאים כעת בתוך גרף ה־ESM החיצוני המפוענח; נכסי מעטפת שאינם חלק מגרף היישום מתועדים בנפרד ב־`docs/compatibility-artifact-inventory.md`.
