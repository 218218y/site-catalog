...
- Python בגרסת ה־major הרשומה ב־`.python-version`, ב־minor הרשום או חדש ממנו, כולל תמיכה ביצירת `venv`. הקובץ מגדיר את קו התאימות המינימלי ואת גרסת ה־CI, לא נעילה שחוסמת minor חדש יותר.
...
יש להתקין את אותו major ואת ה־minor הרשום או חדש ממנו, יחד עם תמיכת `venv`. אם
`python3` של ההפצה עומד בקו הבסיס, אפשר להשתמש בחבילות `python3`, `python3-venv`
ו־`python3-pip`; אם ברירת המחדל ישנה מדי, יש להתקין interpreter מתאים במפורש.
Ruff ו־mypy ממשיכים לכוון לגרסת הבסיס, כדי שקוד חדש לא ישתמש בטעות ב־syntax או API
שאינם זמינים בגרסה המינימלית הנתמכת.
...
מאתר interpreter שעומד בקו הבסיס (`py -3` ב־Windows, או `python3`/`python` בלינוקס,
עם fallback לגרסת הבסיס המפורשת), ויוצר את `.venv` עם אותו runtime שנבחר. מעבר בין
minor נתמך אחד לאחר גורם לבנייה מחדש של `.venv`, כך שאין ערבוב נסתר בין גרסאות.
כך פקודה אינה מצליחה בטעות בגלל חבילה גלובלית שקיימת רק במחשב מסוים.
