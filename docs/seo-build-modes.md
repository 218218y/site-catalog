...
`bundle-site-r2.bat` בונה את האתר הפרטי המלא פעם אחת אל `dist/site-upload-r2` ומעדכן מאותו תוצר את `dist/site-local`. `start-server.bat` בודק את חתימת המקורות לפני ההגשה: תוצר עדכני נפתח בלי בנייה; תוצר ישן מציג בחירה בין עדכון, פתיחת הגרסה הישנה במכוון או ביטול.
