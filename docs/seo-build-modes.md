...
התוצר נשמר ב־`dist/site-public-preview` עם חתימת מקורות ומלאי קבצים. בדיקת
ה־release מוסיפה לצדו חתימת ביקורת, ולכן הרצות חוזרות אינן מוחקות וכותבות מחדש
את מאות הדפים כאשר התוכן וכלי הביקורת לא השתנו.
...
שמות מודולי המסלולים `app-*.js` וקובצי `styles-*.css` בבאנדל כוללים hash תוכן, ולכן שינוי בממשק משנה את ההפניה בכל דף HTML. זו דרישת עקביות מטמון ולא יצירה מחודשת של נתוני הקטלוג. הבונה מחשב את שמות הנכסים לפני רינדור מאות מסלולי ה־SEO וכותב כל דף פעם אחת בלבד. שורת `[timing]` בסיום הבנייה מציגה בנפרד את שלבי הנכסים, הדפים, האימות וה־mirror.
...
אותו שער כלול גם ב־`npm test` וב־`npm run verify`. בהרצה ראשונה או לאחר שינוי
רלוונטי הוא בונה וסורק את כל התוצר; בהרצה חוזרת הוא מאמת את החתימות והמלאי
ומשתמש בתוצאת הביקורת הקשורה לאותו תוכן בדיוק. `npm run build:deploy:public`
מעתיק את התוצר הציבורי שעבר ביקורת לתיקיות ההעלאה והצפייה המקומית בלי לבנות
את הדפים פעם נוספת.
