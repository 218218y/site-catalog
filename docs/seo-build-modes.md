...
`bundle-site-r2.bat` בונה את האתר הפרטי המלא פעם אחת אל `dist/site-upload-r2` ומעדכן מאותו תוצר את `dist/site-local`. `start-server.bat` מגיש את התוצר הקיים בלבד. בדיקת חתימת המקורות והצעת rebuild זמינות במפורש דרך `check-and-start-server.bat`.
