...
for (const html of [template, favorites]) {
  ...
  assert.match(html, /id="favoritesBulkGmail"[^>]*target="_blank"[^>]*tabindex="-1"/);
...
assert.match(app, /function favoriteWorkspaceActionEntries\([\s\S]*?return selectedEntries\.length \? selectedEntries : favoriteWorkspaceVisibleEntries\(entries\)/);
