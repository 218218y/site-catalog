...
assert.match(sharedUi, /managePlaceholder: false/);
assert.match(sharedUi, /placeholderPolicy = IMAGE_PLACEHOLDER_POLICY_PRESERVE_FRAME/);
assert.match(sharedUi, /if \(img\?\.dataset\?\.placeholderPolicy === IMAGE_PLACEHOLDER_POLICY_PRESERVE_FRAME\) return null;/);
assert.match(sharedUi, /function keepSingleViewerPlaceholderReady\(\)/);
