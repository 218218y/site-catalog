...
const controlService = fs.readFileSync(path.join(tools, "catalog_control_service.py"), "utf8");
...
assert.match(controlService, /from catalog_compiler import \(/);
assert.match(controlService, /compile_catalog_outputs_after_source_save/);
assert.match(controlService, /compile_and_write_catalog_data\(/);
assert.doesNotMatch(controlService, /def sync_generated_metadata_after_config_save/);
assert.doesNotMatch(controlService, /def write_catalogs_generated_files/);
assert.doesNotMatch(controlService, /def write_catalogs_search_files/);
assert.doesNotMatch(controlService, /window\.BARGIG_CATALOGS\s*=/);
assert.doesNotMatch(controlService, /window\.BARGIG_CATALOG_SEARCH\s*=/);
