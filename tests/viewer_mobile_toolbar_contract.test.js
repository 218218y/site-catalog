...
assert.match(template, /id="lightboxMobileSearchToggle"[\s\S]*?id="fitHeightBtn"/);
...
assert.match(app, /event\.key === "Escape" && state\.lightboxMobileSearchOpen/);
...
assert.match(css, /@media \(max-width: 480px\)[\s\S]*?grid-template-areas:\s*"brand"\s*"actions";/);
