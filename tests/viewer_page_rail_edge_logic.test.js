...
const hotspotRect = { left: 950, right: 1000, top: 0, bottom: 800, width: 50, height: 800 };
...
assert.equal(geometryApi.isPointInPageRailEdgeActivationZone({ x: 951, y: 100 }), true, 'the full-height strip should use the widened edge zone');
assert.equal(geometryApi.isPointInPageRailEdgeActivationZone({ x: 949, y: 100 }), false, 'points outside the strip should not reveal the rail');
...
navRects.prev = { left: 930, right: 976, top: 377, bottom: 423, width: 46, height: 46 };
assert.equal(geometryApi.isPointInPageRailNavigationConflictZone({ x: 952, y: 400 }), true);
assert.equal(geometryApi.isPointInPageRailEdgeActivationZone({ x: 952, y: 400 }), false, 'an overlapping navigation button must keep its own hit area');
