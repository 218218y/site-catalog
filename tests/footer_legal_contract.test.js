...
const controlService = fs.readFileSync(path.join(root, 'tools', 'catalog_control_service.py'), 'utf8');
...
assert.match(controlService, /def save_footer_content_and_render_pages\(/);
...
assert.match(terms, /<h2>7\. אספקה והובלה<\/h2>/);
assert.match(terms, /כ־14 ימי עסקים/);
assert.match(terms, /עד 45 ימי עסקים/);
assert.match(terms, /<h2>8\. ביטול עסקה, זיכוי והחזר<\/h2>/);
assert.match(terms, /בתוך 14 ימים ממועד קבלת המוצר/);
assert.match(terms, /טרם התחיל תהליך הייצור/);
assert.match(terms, /5% ממחיר העסקה או 100 ₪/);
assert.match(terms, /<h2>9\. אחריות למוצרים<\/h2>/);
assert.match(terms, /האחריות למוצר היא למשך שנה אחת/);
assert.match(terms, /נזקי מים, רטיבות, לחות חריגה או אש/);
assert.match(terms, /18 באוגוסט 2026/);
assert.match(terms, /<h2>18\. יצירת קשר<\/h2>/);
...
assert.match(payment, /אני בן\/בת 18 ומעלה, קראתי ואני מאשר\/ת את/);
...
assert.match(paymentSource, /import \{ copyTextToClipboard, tryNativeShare \} from "\.\.\/js\/22-browser-sharing\.js"/);
assert.match(paymentSource, /tryNativeShare\(\{[\s\S]*?url: link[\s\S]*?mobileOnly: true/);
assert.match(browserSharingSource, /navigator\.clipboard\?\.writeText/);
