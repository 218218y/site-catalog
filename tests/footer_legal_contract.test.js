...
assert.match(css, /\.site-footer-email-row\s*\{[\s\S]*?grid-template-columns:\s*44px minmax\(0, 1fr\)/);
assert.match(css, /\.site-footer-email-actions\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0, 1fr\)/);
assert.match(css, /\.site-footer-contact-list \.site-footer-email-actions a\s*\{[\s\S]*?width:\s*100%[\s\S]*?min-height:\s*31px/);
