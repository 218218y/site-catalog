...
assert.match(navigation, /if \(singleViewerUsesBoundaryPan\(\)\)[\s\S]*?const resetManualView = !isAutoViewerZoom\(\)[\s\S]*?consumeSingleViewerBoundaryInput\(deltaX, deltaY, \{ resetViewOnPageTurn: true \}\)[\s\S]*?boundary\.turned && resetManualView[\s\S]*?holdViewerPageWheelAfterManualReset\(logicalDelta, eventTime\)/);
