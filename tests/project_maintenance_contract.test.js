...
assert.match(pythonVersion, /^\d+\.\d+$/u);
assert.match(ciWorkflow, /python-version-file: \.python-version/);
assert.doesNotMatch(ciWorkflow, /python-version:\s*["']?3\./u);
assert.match(pyproject, new RegExp(`target-version = "py${pythonVersion.replace(".", "")}"`, "u"));
assert.match(pyproject, new RegExp(`python_version = "${pythonVersion.replace(".", "\\.")}"`, "u"));
...
assert.match(pythonLauncher, /major !== required\.major \|\| minor !== required\.minor/);
