...
assert.match(app, /const VIEWER_SCROLL_ZOOM_EXIT_BUFFER_VIEWPORT_RATIO = 0\.36;/);
assert.match(app, /const VIEWER_SCROLL_ZOOM_EXIT_BUFFER_MIN_PX = 144;/);
assert.match(app, /const VIEWER_SCROLL_ZOOM_EXIT_BUFFER_MAX_PX = 330;/);
...
assert.match(app, /function getViewerScrollIsolatedExitBuffer\(\)[\s\S]*?viewportHeight \* VIEWER_SCROLL_ZOOM_EXIT_BUFFER_VIEWPORT_RATIO[\s\S]*?VIEWER_SCROLL_ZOOM_EXIT_BUFFER_MIN_PX[\s\S]*?VIEWER_SCROLL_ZOOM_EXIT_BUFFER_MAX_PX/);
assert.match(app, /function clampSinglePan\(\)[\s\S]*?const verticalExitBuffer = getViewerScrollIsolatedExitBuffer\(\);[\s\S]*?metrics\.overflowY \+ verticalExitBuffer/);
assert.match(app, /viewerScrollPointerHandoff: null/);
assert.match(app, /function consumeViewerScrollIsolatedPan\(deltaX = 0, deltaY = 0\)/);
assert.match(app, /remainingDeltaY: safeDeltaY - consumedDeltaY/);
assert.match(app, /hasVerticalExitIntent: Math\.abs\(safeDeltaY\) > Math\.abs\(safeDeltaX\) \* 0\.5/);
assert.match(app, /function panViewerScrollIsolatedZoomByWheel\(deltaX = 0, deltaY = 0\)[\s\S]*?consumeViewerScrollIsolatedPan\(deltaX, deltaY\)/);
assert.match(app, /function beginViewerScrollPointerHandoff\(event, deltaX = 0, deltaY = 0\)/);
assert.match(app, /exitViewerScrollIsolatedZoom\(\{ restorePage: true, nextZoom: AUTO_VIEWER_ZOOM \}\);[\s\S]*?state\.viewerScrollPointerHandoff = \{/);
assert.match(app, /function continueViewerScrollPointerHandoff\(event\)[\s\S]*?handoff\.lastY - event\.clientY[\s\S]*?scheduleViewerScrollPointerHandoffFlush\(\)/);
assert.match(app, /function movePointerInteraction\(event\)[\s\S]*?continueViewerScrollPointerHandoff\(event\)[\s\S]*?const scrollDeltaY = previousPoint\.y - event\.clientY;[\s\S]*?beginViewerScrollPointerHandoff\(event, 0, result\.remainingDeltaY\)/);
assert.match(app, /!els\.lightboxImageFrame\?\.contains\(event\.target\)[\s\S]*?&& !isTouchLikePointer\(event\)/);
assert.match(app, /function endPointerInteraction\(event\)[\s\S]*?finishViewerScrollPointerHandoff\(event\)/);
assert.match(app, /function cancelPointerInteraction\(event\)[\s\S]*?finishViewerScrollPointerHandoff\(event\)/);
...
assert.match(css, /\.viewer-scroll-pages\s*\{[\s\S]*?gap:\s*clamp\(42px, 6vh, 66px\);/);
assert.match(css, /@media \(max-width: 760px\) \{[\s\S]*?\.viewer-scroll-pages\s*\{[\s\S]*?gap:\s*27px;/);
...
assert.match(css, /\.lightbox\.viewer-touch-handoff-active \.viewer-scroll-pages\s*\{[\s\S]*?scroll-snap-type:\s*none;/);
