'use strict';

const assert = require('node:assert/strict');
const fs = require('node:fs');
const path = require('node:path');

const root = path.join(__dirname, '..');
const panel = fs.readFileSync(path.join(root, 'catalog-control-panel.html'), 'utf8');
const panelApp = fs.readFileSync(path.join(root, 'src', 'control-panel', 'catalog-control-panel.js'), 'utf8');
const panelCss = fs.readFileSync(path.join(root, 'src', 'control-panel', 'catalog-control-panel.css'), 'utf8');
const server = fs.readFileSync(path.join(root, 'tools', 'catalog_control_server.py'), 'utf8');
const editor = fs.readFileSync(path.join(root, 'tools', 'taxonomy_editor.py'), 'utf8');

assert.match(panel, /<h2>ניהול קטגוריות וכתובות SEO<\/h2>/);
assert.match(panel, /href="\/src\/control-panel\/catalog-control-panel\.css"/);
assert.match(panel, /src="\/src\/control-panel\/catalog-control-panel\.js"/);
assert.doesNotMatch(panel, /<style>|<script(?![^>]*src=)/);
assert.match(panel, /id="taxonomyCategories"/);
assert.match(panel, /id="taxonomySubcategories"/);
assert.match(panel, /id="saveTaxonomy"/);
assert.match(panelApp, /function reconcileTaxonomyDraftFromCatalogs/);
assert.match(panelApp, /function renderTaxonomyEditor/);
assert.match(panelApp, /api\('\/api\/taxonomy'/);
assert.match(panelApp, /taxonomy: taxonomyPayload\(\)/);
assert.match(panelApp, /routeLockUpdates/);
assert.match(panelApp, /נעילת כתובות SEO עודכנה אוטומטית/);
assert.match(panelApp, /placeholder="חסר — לדוגמה dining-tables"/);
assert.match(panelCss, /grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/);
assert.equal((panel.match(/class="taxonomy-panel-scroll"/g) || []).length, 2);
assert.match(panelCss, /\.taxonomy-panel-scroll \{[\s\S]*overflow-x: auto;/);
assert.match(panelCss, /\.taxonomy-list \{[^}]*min-width: 680px;/);
assert.match(panelApp, /\['bundle_r2', 'cloudflare_pages_deploy'\]/);

assert.match(server, /if path == "\/api\/taxonomy"/);
assert.match(server, /atomic_write_catalogs_and_taxonomy/);
assert.match(server, /compile_catalog_outputs_after_source_save/);
assert.match(server, /taxonomy_action_availability/);
assert.match(server, /"taxonomy": taxonomy/);
assert.match(server, /append_new_configured_routes_to_lock/);
assert.match(server, /routeLockUpdates/);
assert.match(server, /route_lock_sync_warnings/);

assert.match(editor, /def reconcile_taxonomy_with_catalogs/);
assert.match(editor, /def apply_taxonomy_renames_to_catalogs/);
assert.match(editor, /def taxonomy_completion_issues/);
assert.match(editor, /"slug": ""/);
assert.match(editor, /"description": ""/);

console.log('catalog_taxonomy_control_panel_contract.test.js: PASS');
