...
assert.match(appShell, /function attachFeatureEvents\(\) \{[\s\S]*?requireFeatureInterface\("catalog-grid"\)[\s\S]*?requireFeatureInterface\("search"\)[\s\S]*?requireFeatureInterface\("favorites"\)/);
assert.match(appShell, /bindFeatureEventsOnce\("catalog-grid", catalogGrid\.attachEvents\)/);
assert.match(appShell, /bindFeatureEventsOnce\("search-ui", search\.attachEvents\)/);
assert.match(appShell, /bindFeatureEventsOnce\("favorites-share", favorites\.attachEvents\)/);
assert.match(appShell, /getFeatureInterface\("viewer"\)[\s\S]*?viewer\.attachEvents/);
