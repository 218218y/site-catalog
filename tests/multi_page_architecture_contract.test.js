...
assert.match(searchSource, /searchElements\.globalSearchOpen\?\.addEventListener\("click"[\s\S]*?setGlobalSearchPanelOpen/);
...
assert.match(viewerSource, /navigateTo\(viewerDocumentUrl\(catalog\.id, page, \{ source \}\)\)/);
assert.match(favoritesShareSource, /navigateTo\(favoritesDocumentUrl\(\)\)/);
assert.match(appShellSource, /siteRoutes\?\.parseLocation/);
...
assert.match(navigationSource, /let currentAppPage = siteRoutes\?\.pageFromLocation/);
assert.match(navigationSource, /function setCurrentAppPage\([\s\S]*?document\.body\.dataset\.page = currentAppPage/);
assert.match(navigationSource, /function isInternalAppDocumentUrl\([\s\S]*?siteRoutes\?\.isSameAppDocumentLocation\?\.\(window\.location, url, currentAppPage\)/);
assert.match(navigationSource, /function canNavigateWithinCurrentDocument\([\s\S]*?getFeatureInterface\("viewer"\)\?\.usesInDocumentFullscreenNavigation\?\.\(\)[\s\S]*?isInternalAppDocumentUrl\(url\)/);
...
assert.match(navigationSource, /function navigateWithinCurrentDocument\([\s\S]*?history\.pushState\([\s\S]*?requireFeatureInterface\("app-shell"\)\.renderRoute\(\{ scrollPosition/);
assert.match(navigationSource, /function navigateTo\([\s\S]*?canNavigateWithinCurrentDocument\(targetUrl\)[\s\S]*?window\.location\.replace\(targetUrl\?\.href \|\| target\)[\s\S]*?window\.location\.assign\(targetUrl\?\.href \|\| target\)/);
assert.match(navigationSource, /function handleInternalAppLinkClick\([\s\S]*?getFeatureInterface\("viewer"\)\?\.usesInDocumentFullscreenNavigation\?\.\(\)[\s\S]*?navigateWithinCurrentDocument\(targetUrl\)/);
assert.match(navigationSource, /window\.addEventListener\("popstate"[\s\S]*?requireFeatureInterface\("app-shell"\)\.renderRoute\(\{/);
...
assert.match(navigationSource, /function markAppReady\(\) \{[\s\S]*?data-app-ready/);
assert.match(appShellSource, /return initDocumentRoute\(\)/);
