...
assert.match(app, /id: "inquiry"[\s\S]*?target: \(\) => getFeatureInterface\("inquiry"\)\?\.onboardingTarget\(\) \|\| null[\s\S]*?floatingTargets: \(\) => \{[\s\S]*?getFeatureInterface\("inquiry"\)\?\.onboardingTarget\(\)[\s\S]*?getFeatureInterface\("favorites"\)\?\.onboardingTarget\(\)/);
