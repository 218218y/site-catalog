...
test("favorites workspace supports notes, ordering, filtering, focused sharing, and the shared inquiry dialog", async ({ page, browser }) => {
  ...
  await expect(page.locator("#viewerInquiryOverlay")).toHaveClass(/visible/);
  await expect(page.locator("#viewerInquiryTitle")).toHaveText("בירור על הדגמים");
  await expect(page.locator("#viewerInquiryPage")).toContainText("כל המועדפים");
  await expect(page.locator("#viewerInquiryCountBadge")).toHaveText("+2");
  await expect(page.locator("#viewerInquiryShareLabel")).toHaveText("שיתוף פרטי הדגמים");
  await expect(page.locator("#viewerInquiryCopyLabel")).toHaveText("העתקת ההודעה והקישורים");
  await expect(page.locator("#viewerInquiryActions .viewer-inquiry-action")).toHaveCount(4);
  const allItemsGmailHref = await page.locator("#viewerInquiryGmail").getAttribute("href");
  const allItemsEmailHref = await page.locator("#viewerInquiryEmail").getAttribute("href");
  const allItemsBody = new URL(allItemsGmailHref).searchParams.get("body") || "";
  const allItemsEmailBody = new URL(allItemsEmailHref).searchParams.get("body") || "";
  expect(allItemsBody).toContain("לבדוק ברוחב 180");
  expect(allItemsEmailBody).toContain("לבדוק ברוחב 180");
  ...
  await expect(page.locator("#favoritesInquiryLabel")).toHaveText("בירור על הדגם");

  await page.locator("#favoritesInquiryButton").click();
  await expect(page.locator("#viewerInquiryPage")).toContainText("הדגמים שנבחרו");
  ...
  expect((copiedInquiry.match(/https?:\/\//g) || []).length).toBe(2);

  await page.evaluate(() => { window.__bargigE2eShare = null; });
  await page.locator("#favoritesInquiryButton").click();
  await page.locator("#viewerInquiryShare").click();
  await expect.poll(() => page.evaluate(() => window.__bargigE2eShare || null)).not.toBeNull();
  const sharedInquiry = await page.evaluate(() => window.__bargigE2eShare || null);
  expect(sharedInquiry.text).toContain("לבדוק ברוחב 180");
  expect((sharedInquiry.text.match(/https?:\/\//g) || []).length).toBe(2);
  expect(sharedInquiry.url).toContain("favorites.html?selection=");
  ...
  await page.locator("#favoritesInquiryButton").click();
  await expect(page.locator("#viewerInquiryPage")).toContainText("כל המועדפים");
  const filteredGmailHref = await page.locator("#viewerInquiryGmail").getAttribute("href");
  const filteredBody = new URL(filteredGmailHref).searchParams.get("body") || "";
  expect((filteredBody.match(/https?:\/\//g) || []).length).toBe(3);
  await page.locator("#viewerInquiryClose").click();
