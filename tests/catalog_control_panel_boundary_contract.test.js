...
const serviceBoundary = read("tools", "catalog_control_service.py");
const fileBoundary = read("tools", "catalog_control_files.py");
const jobBoundary = read("tools", "catalog_control_jobs.py");
...
assert.ok(server.split(/\r?\n/).length <= 700, "Control server must remain a thin HTTP composition root");
assert.doesNotMatch(server, /def save_catalogs_transactionally\(/);
assert.doesNotMatch(server, /def read_multipart_pdf_upload\(/);
assert.doesNotMatch(server, /def run_job\(/);
assert.match(serviceBoundary, /def save_catalogs_transactionally\(/);
assert.match(serviceBoundary, /ProjectTransaction/);
assert.match(fileBoundary, /def read_multipart_pdf_upload\(/);
assert.match(fileBoundary, /def save_uploaded_pdf\(/);
assert.match(jobBoundary, /def run_job\(/);
assert.match(jobBoundary, /def cancel_job\(/);
