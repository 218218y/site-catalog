...
assert.match(searchUi, /GLOBAL_SEARCH_INITIAL_RENDER_COUNT = 3/);
assert.match(searchUi, /GLOBAL_SEARCH_RENDER_CHUNK_SIZE = 3/);
assert.match(searchUi, /renderGlobalSearchResultsProgressively/);
assert.match(searchUi, /window\.requestAnimationFrame\(appendNextBatch\)/);
assert.match(searchUi, /window\.cancelAnimationFrame\(globalSearchAppendFrame\)/);
