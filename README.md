...
דף המועדפים תומך בהערות פרטיות לכל דגם, סינון לפי קטלוג, שינוי סדר בגרירה או בכפתורי מעלה/מטה, בחירה מרובה ושיתוף רשימה מרוכזת. בחירה של 2–4 פריטים מאפשרת השוואה זה לצד זה ובירור מרוכז דרך Gmail. ההערות והסדר נשמרים ב־`localStorage` במכשיר בלבד. קישור רשימת המועדפים ממשיך להכיל רק מזהי קטלוגים ועמודים — ללא הערות וללא הסדר המקומי; שיתוף הודעה מרוכזת כולל הערות רק כאשר המשתמש מפעיל אותו במפורש.
