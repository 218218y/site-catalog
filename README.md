...
לא עורכים ידנית את קובצי ה־HTML שנוצרו. עורכים את התבניות והחלקים המשותפים, ואז מריצים preview מלא:
...
הבנייה יוצרת מחדש את כל מבנה הכתובות בתוך `dist/site-local`; תיקיית המקור נשארת נקייה ואינה משמשת כ־web root. קובצי ה־shell הטכניים `catalog.html` ו־`viewer.html` נשארים רק במקור לצורכי בדיקות ותבניות ואינם נכללים באתר המלא. טקסט הפוטר נערך בדרך כלל דרך לוח השליטה; השמירה מעדכנת את קובץ התוכן ובונה מיד מחדש את הדפים המשותפים.
...
ההרצה הבאה של `start-server.bat`, `npm run build:local` או בניית באנדל Cloudflare
