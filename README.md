...
הפקודה בונה, רק כאשר נדרש, את `dist/site-upload-r2` ומעתיקה מאותו תוצר מאומת את `dist/site-local`. לאחר מכן `start-server.bat` רק מגיש את `dist/site-local` הקיים ואינו מפעיל בנייה או לוח שליטה. תיקיית המקור נשארת נקייה ואינה משמשת כ־web root. קובצי ה־shell הטכניים `catalog.html` ו־`viewer.html` נשארים רק במקור לצורכי בדיקות ותבניות ואינם נכללים באתר המלא.
...
ההרצה הבאה של `bundle-site-r2.bat` או `npm run build:local`
...
קטלוג חדש שמשויך לקטגוריה קיימת אינו דורש עבודת SEO נוספת. קטגוריה או
תת־קטגוריה חדשה דורשת פעם אחת שם, slug ותיאור ב־`catalog-taxonomy.config.json`;
המערכת אינה מנחשת טקסט שיווקי או כתובת. קטגוריה ללא קטלוגים מושמטת אוטומטית.
...
לצדן נשמרים קובצי `*.build.json` עם חתימות המקורות ומלאי הקבצים. הם נמצאים מחוץ לתיקיית האתר ואינם מועלים. `dist/site-e2e` נוצר רק בעת בדיקות Playwright, ו־`dist/site-public-preview` רק בבדיקת מצב public מפורשת. התיקיות הישנות `dist/seo-private` ו־`dist/seo-public` אינן בשימוש ומנוקות בבנייה הרגילה.
