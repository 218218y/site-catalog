...
מפת האחריות, כיוון התלויות וכללי התחזוקה מפורטים ב־`docs/frontend-architecture.md`. סדר ה־CSS נועל כעת את הדרכת ה־Viewer כ־override מאוחר לפני שכבת הנגישות; פירוט הפחתת `!important`, ההוכחות ורשימת הבדיקות הידניות נמצאים ב־`docs/CSS_CASCADE_OWNERSHIP_CLOSEOUT.md`.
...
האתר כולל מערכת ניטור מצומצמת ושומרת פרטיות. הממשק שולח אירועים מאושרים בלבד אל `/api/telemetry`, ו־Cloudflare Pages Function שומר אותם ב־Workers Analytics Engine. המערכת מודדת ביקורי אפליקציה כמכנה תפעולי, פתיחת קטלוגים, חיפושים שהושלמו ותוצאותיהם, שימוש במועדפים, פעולות יצירת קשר, שגיאות JavaScript/תמונה ובדפדפנים תומכים גם LCP, INP ו־CLS. אירועי CLS כוללים token רכיב מרשימה סגורה, ואירועי תמונה כוללים משטח, נראות ו־request correlation קצר־חיים שנוצר לכל lifecycle ואינו מזהה משתמש. הקלדה חלקית אינה נרשמת כחיפוש; אירוע נוצר רק בשליחה עם Enter או בפתיחת תוצאה. המערכת אינה יוצרת עוגיות או מזהה מבקר מתמשך, ואינה שולחת IP, User-Agent, referrer מלא, selector חופשי, טקסט DOM או stack של שגיאה. Global Privacy Control ו־Do Not Track מכבים את המדידה בדפדפן. הדוח המקומי מציג אחוזון 75, מכנים אמיתיים, שיעורים לכל 1,000 ביקורים, cohorts לפי גרסה/מסלול/מסך ורכיב, ומגמות מול התקופה הקודמת.
...
אפשר להוסיף `--format json` ליצוא JSON, `--console` לתצוגת הטקסט הישנה, `--output-dir PATH` לשינוי תיקיית היעד, או `--current-release deploy-...` להגדרה מפורשת של גרסת הפרודקשן הנוכחית. מומלץ לשמור את אותה גרסה גם ב־`BARGIG_CURRENT_RELEASE_ID` בתוך `telemetry.env`; כאשר הערך חסר, הכלי אינו מנחש איזו פריסה היא הנוכחית. קובצי הדוח מוחרגים מ־Git משום שהם עשויים להכיל נתונים עסקיים מצטברים.

כלי הדוח שולח ל־Analytics Engine 21 שאילתות `SELECT` תחומות — אחת לכל אזור או cohort בדוח — ומאחד את התוצאות מקומית. אין שימוש ב־`UNION ALL` או ב־CTE. השאילתות כוללות ביקורי אפליקציה לפי גרסה/עמוד/מסלול/מסך, RUM לפי רכיב, שיעורי אמינות, ותוצאות lifecycle של תמונות לפי משטח ונראות. שורות אבחון מקובצות לפי עמודות dataset ממשיות ומקבלות כותרת קריאה במחשב המקומי.
