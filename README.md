# העלאה מהירה ל-Cloudflare R2

להעלאת הרבה תמונות קטלוגים לא מומלץ להשתמש ב-Wrangler קובץ-קובץ. זה איטי, וכל קובץ מריץ תהליך נפרד.
הדרך המומלצת בפרויקט הזה היא סנכרון דרך S3 API עם AWS CLI:

1. התקן AWS CLI v2.
2. ב-Cloudflare R2 צור Access Key / API Token עם הרשאת Object Read and Write לבאקט `bargig-catalog`.
3. הרץ פעם אחת:

```bat
setup-r2-aws-profile.bat
```

4. אחרי יצירת `dist\r2-assets` הרץ:

```bat
sync-r2-assets-fast.bat bargig-catalog
```

הסקריפט משתמש ב-`aws s3 sync` מול endpoint של R2:

```text
https://7d352c315748f2f8c6e723c5fc46f606.r2.cloudflarestorage.com
```

הכתובת הזו היא S3 API להעלאה בלבד. באתר עצמו ממשיכים להשתמש בכתובת הציבורית:

```text
https://pub-5e6c7421563f4086ba1e097bb88f3348.r2.dev
```

## האם קבצים קיימים עולים מחדש?

בהרצה רגילה: לא.

- `build-r2-assets.bat` שומר קטלוגים שכבר הומרו ל-WebP ולא בונה אותם מחדש אם לא ביקשת `--force`.
- `sync-r2-assets-fast.bat` משתמש ב-`aws s3 sync`, ולכן הוא מדלג על אובייקטים שכבר קיימים מרחוק ולא השתנו.
- אם הוספת קטלוג חדש, יעלו בעיקר הקבצים החדשים.
- אם הרצת המרה בכוח ויצרת מחדש את כל התמונות, ייתכן שה-sync יעלה יותר קבצים כי זמני הקבצים השתנו. במקרה נדיר כזה אפשר להריץ:

```bat
sync-r2-assets-fast.bat bargig-catalog --size-only
```

אבל `--size-only` פחות מחמיר: אם תמונה השתנתה ובמקרה נשארה בדיוק באותו גודל, הוא עלול לדלג עליה. לכן משתמשים בזה רק כשברור שהתוכן לא השתנה ורק הזמנים השתנו.

## מחיקה מרחוק

ברירת המחדל לא מוחקת כלום מה-R2. אם הסרת קטלוגים ורוצה למחוק גם מהבאקט, הרץ בזהירות:

```bat
sync-r2-assets-fast.bat bargig-catalog --delete
```

## Wrangler

`upload-r2-assets-wrangler.bat` נשאר בפרויקט כגיבוי לתיקונים קטנים או למי שלא רוצה להגדיר AWS CLI. להעלאה גדולה של קטלוגים — השתמש ב-`sync-r2-assets-fast.bat`.
