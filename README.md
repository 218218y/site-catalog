...
- לערוך כותרת, קטגוריה, תת־קטגוריה והגדרת OCR לכל קטלוג.
...
הדף לא נועד לעלות לאתר; הוא כלי עבודה מקומי בלבד. השרת מפעיל רק רשימת פעולות קבועה מראש, ולא מריץ פקודות חופשיות מהדפדפן.
...
אחרי הסריקה ערוך ב-`catalogs.config.json` את `title`, `description`, `category`, `subcategory` ו-`ocr` לפי הצורך. `category` קובע את הכפתורים הראשיים בראש האתר. `subcategory` הוא אופציונלי: אם הוא ריק או חסר, הקטלוג נשאר מסודר כמו היום בתוך הקטגוריה הראשית; אם יש ערך, הקטלוג יוצג גם בתוך תת־קטגוריה עם כפתור פנימי לגלילה וסימון.
...
catalogs.config.json        רשימת הקטלוגים; קבצי PDF חדשים מתווספים אליו בהרצת sync-catalog-pdfs.bat; השדה ocr שולט על OCR פר קטלוג
...
tools/catalog_control_server.py  שרת מקומי שמפעיל פעולות קטלוגים קבועות ומעדכן את catalogs.config.json
