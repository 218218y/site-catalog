...
אחרי העלאת production הכלי אינו פונה לכתובת הזמנית `*.pages.dev`, משום שכתובות אלו עשויות להיחסם ברשת מסוננת. האימות הציבורי מתבצע רק דרך `https://bargig-furniture.com`: הכלי ממתין לעדכון הדומיין ומוודא שהוא מפנה בדיוק לדור ה־CSS/JS שנבנה עכשיו. בדיקות השלמות המקומיות של הבאנדל מתבצעות תמיד לפני ההעלאה. בפריסת preview אין אימות רשת אוטומטי, משום שהכתובת הציבורית היחידה שלה היא כתובת `pages.dev`. במקרה תחזוקה שבו רוצים לדלג גם על בדיקת דומיין ה־production אפשר להוסיף `--no-verify`.
