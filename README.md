...
סביבת בדיקות ה־Python נשמרת מקומית בתוך `.venv` ואינה תלויה בחבילות שמותקנות ב־Python הכללי של Windows. בהרצה הראשונה אפשר להכין אותה במפורש:

```bat
npm run setup:python
```

אין חובה להפעיל את `.venv` ידנית. גם `npm test` וגם `npm run verify` בודקים את הסביבה לפני ההרצה, יוצרים אותה במידת הצורך ומתקינים רק כאשר קובצי הדרישות השתנו או שחבילה חסרה.

לאחר שינוי בקוד המקור אפשר להריץ בדיקה מהירה אחת, ללא תלות בפקודות `for` של Windows:

```bat
npm test
```

הפקודה בודקת שהבאנדלים מעודכנים, מאמתת תחביר JavaScript ומריצה את כל בדיקות JavaScript ו־Python.
לפני העלאה מומלץ להריץ את הבדיקה המלאה:

```bat
npm run verify
```

הבדיקה המלאה יוצרת גם חבילת Cloudflare Pages נקייה בתיקייה זמנית, מאמתת אותה ומוחקת את התיקייה בסיום. ל־npm אין ספריות JavaScript חיצוניות בפרויקט; `package.json` משמש כממשק פקודות אחיד וגם מפעיל את הכנת סביבת Python המקומית.
