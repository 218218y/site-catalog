...
אחרי הסריקה ערוך ב-`catalogs.config.json` את `title`, `description`, `category` ו-`subcategory` לפי הצורך. `category` קובע את הכפתורים הראשיים בראש האתר. `subcategory` הוא אופציונלי: אם הוא ריק או חסר, הקטלוג נשאר מסודר כמו היום בתוך הקטגוריה הראשית; אם יש ערך, הקטלוג יוצג גם בתוך תת־קטגוריה עם כפתור פנימי לגלילה וסימון.
...
אם יש קטלוגים בלי כיתוב אמיתי, או קטלוגים שבהם OCR עלול לזהות שיבושים מתוך התמונה, אפשר להמיר רק קטלוגים חדשים/חסרים בלי OCR בכלל:

```bat
convert-catalogs-no-ocr.bat
```

הפקודה הזו יוצרת את תמונות הקטלוג ואת נתוני הקטלוגים בלי להריץ OCR. היא עדיין מדלגת על קטלוגים שכבר הומרו, ובהרצה רגילה עתידית של `convert-catalogs.bat` הם לא ייבנו מחדש רק בגלל שהפקודה הרגילה עובדת עם OCR במצב `auto`.
